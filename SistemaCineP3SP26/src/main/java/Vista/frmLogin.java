...
private void abrirContenedorPrincipal() {
    String sAreaSeleccionada;
    sAreaSeleccionada = cboOpciones.getSelectedItem().toString();

   
    switch (sAreaSeleccionada) {
        case "Seguridad":
            try {
                // Cambia 'MdiSeguridad' por el nombre real de tu clase Contenedor
                MdiSeguridad menu = new MdiSeguridad(); 
                menu.setVisible(true);
                this.dispose(); // Cierra el login
            } catch (Exception e) {
                System.out.println("Error al abrir MDI: " + e);
            }
            break;
    ...
    private void btnAceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAceptarActionPerformed
        ...
        if (txtUsuario.getText().trim().isEmpty() || String.valueOf(txtContraseña.getPassword()).trim().isEmpty()) {
            ...
            try {
                ...
                String sPassInput = String.valueOf(txtContraseña.getPassword());
                
                usuario.setUsuNombre(txtUsuario.getText().trim());
                // Buscamos el usuario en la BD (Nota: usamos el método estandarizado con 'm')
                usuario = usuario.mBuscarUsuarioPorNombre(usuario);

                // Verificación de credenciales
                if (usuario != null && seguridad.encode(sPassInput).equals(usuario.getUsuContrasena())) {
                    JOptionPane.showMessageDialog(null, "Bienvenido al SISTEMA\n", "Mensaje de bienvenida", JOptionPane.INFORMATION_MESSAGE);
                    
                    // Registro de usuario conectado
                    clsUsuarioConectado.setUsuId(usuario.getUsuId());
                    clsUsuarioConectado.setUsuNombre(usuario.getUsuNombre());

                    // Registro en Bitácora
                    BitacoraDAO bitacora = new BitacoraDAO();
                    bitacora.insert(usuario.getUsuId(), codigoAplicacion, "Inicio Sesión");
                    
                    // Redirección por departamento
                    abrirContenedorPrincipal();
                } else {
